...
class AdminRentalSerializer(serializers.Serializer):
    ...
    def get_overdue_amount(self, obj):
        if obj.ended_at:
            return Decimal(str(obj.overdue_amount or Decimal('0'))).quantize(Decimal('0.01'))

        if obj.status in ['ACTIVE', 'OVERDUE'] and obj.due_at and timezone.now() > obj.due_at:
            return Decimal(str(obj.current_overdue_amount or Decimal('0'))).quantize(Decimal('0.01'))
