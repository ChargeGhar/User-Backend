...
from api.common.utils.signature import SignChargeGharMain
...
class SignatureTestCase(TestCase):
    ...
    def test_signature_validation_success(self):
        ...
        is_valid, error = self.sign_util.validate_signature(payload, timestamp, signature, time_tolerance=10)
    ...
    def test_signature_validation_invalid_signature(self):
        ...
        is_valid, error = self.sign_util.validate_signature(payload, timestamp, "invalid-signature", time_tolerance=10)
    ...
    def test_signature_validation_expired_timestamp(self):
        ...
        signature = self.sign_util.generate_signature(payload, timestamp)
        is_valid, error = self.sign_util.validate_signature(payload, timestamp, signature, time_tolerance=300)
...
class StationSyncAPITestCase(TestCase):
    ...
    def test_full_station_sync_without_signature(self):
        ...
        payload = {
            "type": "full",
            "timestamp": int(time.time()),
            "device": {"serial_number": "TEST12345", "status": "ONLINE"},
            "station": {"total_slots": 2},
            "slots": [],
            "power_banks": []
        }
    ...
    def test_full_station_sync_with_valid_signature(self):
        ...
        with self.settings(IOT_SYSTEM_SIGNATURE_SECRET=self.secret_key):
            payload_data = {
                ...
                "station": {"total_slots": 2},
                "slots": [
                    {"slot_number": 1, "status": "AVAILABLE", "battery_level": 0, "slot_metadata": {}},
                    {"slot_number": 2, "status": "OCCUPIED", "battery_level": 85, "power_bank_serial": "PB12345", "slot_metadata": {}}
                ],
                "power_banks": [
                    {"serial_number": "PB12345", "status": "AVAILABLE", "battery_level": 85, "current_slot": 2, "hardware_info": {"temperature": 30, "voltage": 5000}}
                ]
            }
