...
class FranchiseVendorService(BaseService):
    ...
    def create_vendor(
        ...
        """
        Create a new vendor under franchise with multiple stations.
        ...
        4. Assign stations (FRANCHISE_TO_VENDOR)
        ...
        Args:
            ...
            station_ids: List of Station UUIDs to assign (must be franchise's stations)
        ...
        - BR2.3: Vendor can have MULTIPLE stations
        ...
        # Verify all stations belong to franchise and are available
        for station_id in station_ids:
            ...
            if not station_owned:
                raise ServiceException(
                    detail=f"Station {station_id} not found or not owned by franchise",
            ...
            if StationDistributionRepository.station_has_operator(station_id):
                raise ServiceException(
                    detail=f"Station {station_id} already has a vendor assigned",
        ...
        # Create station distributions (FRANCHISE_TO_VENDOR)
        for station_id in station_ids:
        ...
        self.log_info(f"Vendor created: {partner.code} ({vendor_type}) with {len(station_ids)} stations by franchise {franchise.id}")
        return partner
    ...
    def get_vendors_list(self, franchise: Partner, filters: Dict) -> Dict:
        ...
        for vendor in paginated['results']:
            ...
            vendor_distributions = vendor.station_distributions.filter(
                distribution_type=StationDistribution.DistributionType.FRANCHISE_TO_VENDOR,
                is_active=True
            ).select_related('station')

            stations = []
            for d in vendor_distributions:
                stations.append({
                    'id': str(d.station.id),
    ...
    def get_vendor_detail(self, franchise: Partner, vendor_id: str) -> Dict:
        ...
        # Get all station assignments
        vendor_distributions = StationDistribution.objects.filter(
        ...
        for d in vendor_distributions:
            stations.append({
                ...
                'address': d.station.address,
                'status': d.station.status,
                'total_slots': d.station.total_slots,
            })

        # Get revenue share (all stations share same model)
        revenue_share = None
        first_dist = vendor_distributions.first()
        if vendor.vendor_type == Partner.VendorType.REVENUE and first_dist:
            share = StationRevenueShare.objects.filter(
                distribution_id=first_dist.id
            ).first()
            if share:
                revenue_share = {
